...
export default function MapScreen() {
  ...
  // Screen Focus Listener — auto-focus on Safe Zone when returning to map
  useFocusEffect(
    useCallback(() => {
  ...
  const centerOnUser = async () => {
    if (!userLocation) {
      try {
        ...
        mapRef.current?.animateToRegion({ ...coord, latitudeDelta: 0.005, longitudeDelta: 0.005 }, 1000);
      } catch (err) {
      ...
      [
        ...
        { 
          ...
          onPress: () => navigation.navigate('Animals', { 
            screen: 'AnimalDetail', 
            params: { mode: 'create', initialLocation: { latitude, longitude }, initialZoneId: matchingZoneId } 
          }) 
        },
      ]
    );
      ...
      >
        ...
        {geofences.map((gf) => {
          if (gf.type === 'polygon' && gf.polygon_coords) {
            ...
            return (
              <Polygon
                key={`poly-${gfId}-${isSelected}`}
                coordinates={coords}
                fillColor={isSelected ? 'rgba(255, 255, 0, 0.35)' : (gf.fill_color ? `${gf.fill_color}33` : 'rgba(79, 70, 229, 0.2)')}
        ...
        {/* Zone Center Markers with Labels */}
        {geofences.map((gf) => {
            ...
            >
              <View style={styles.zoneMarkerWrapper}>
                <View style={[
                  ...
                  isSelected && { transform: [{ scale: 1.2 }], borderColor: '#FFFF00' }
                ]}>
                  <Ionicons 
                    name={isSelected ? "location-sharp" : "location"} 
                    size={isSelected ? 22 : 18} 
                    color={isSelected ? '#FFFF00' : (gf.is_primary ? COLORS.success : COLORS.primary)} 
                  />
                </View>
                {gf.name && (
        ...
        {userLocation && (
          <Marker coordinate={userLocation} title="Ma Position" flat={true} zIndex={100}>
        ...
        {/* Animal Markers (filtered by zone if one is selected) */}
        {animals
          .filter((animal) => {
            if (selectedAnimal?.isZone) {
              return String(animal.current_zone_id || '') === String(selectedAnimal.id || '');
            }
            return true;
          })
          .map((animal) => (
        ...
        <View style={styles.modalOverlay}>
          ...
          <View style={styles.zonesBottomSheet}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                const zId = item.id || item._id;
                const animalsInside = animals.filter(a => String(a.current_zone_id || '') === String(zId)).length;
                return (
                  <TouchableOpacity style={styles.zoneQuickCard} onPress={() => focusZone(item)}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.zoneQuickName}>{item.name || `Zone #${zId}`}</Text>
          ...
          <View style={styles.sheetHeader}>
            ...
            >
              <Text style={styles.sheetBadgeText}>
                {selectedAnimal.isZone ? 'CLEAR FILTER' : (selectedAnimal.status || 'unknown').toUpperCase()}
              </Text>
            </TouchableOpacity>
          </View>
